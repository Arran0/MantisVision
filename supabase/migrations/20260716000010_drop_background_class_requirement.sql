-- Stop forcing a "background_class" (no-subject class) onto any
-- measurement. Previously the schema required at least one classification
-- to declare one, and seaweed_presence's "No" class was preset as it by
-- default; there was no admin UI for it at all beyond that hardcoded seed.
-- An admin creating their own plain classification (e.g. their own
-- seaweed_presence with Yes/No classes) shouldn't be forced to also wire up
-- this concept — it's now fully optional, with no UI and no validation
-- requirement. seaweed_presence in this schema is a plain classification,
-- same as any other.
--
