-- Turn species into a normal classification measurement instead of a
-- schema-level species[]/active_species_slug concept.
--
-- Previously the schema carried a top-level list of species plus a single
-- "active" one — every dataset image and the deployed model were implicitly
-- scoped to that one active species, and switching species meant flipping a
-- schema-wide toggle. There is no reason for that restriction now that every
-- other quality is a per-image column: species becomes a required, locked,
-- extensible classification measurement ("species") just like "disease" —
-- one class per species, applies only when seaweed_presence == "Yes", and
-- the admin adds a class per new species instead of managing a separate
-- species list with one flagged "active".
--
